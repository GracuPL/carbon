...
const NoQuoteReasonForm = ({
  ...
  useEffect(() => {
    ...
    if (fetcher.state === "loading" && fetcher.data?.data) {
      onClose?.();
      toast.success(t("createdNoQuoteReason"));
    } else if (fetcher.state === "idle" && fetcher.data?.error) {
      toast.error(
        t("failedToCreateNoQuoteReason", { message: fetcher.data.error.message })
      );
    }
      ...
      >
        <ModalDrawerContent>
          ...
          >
            <ModalDrawerHeader>
              <ModalDrawerTitle>
                {isEditing ? t("sales:editReason") : t("sales:newNoQuoteReason")}
              </ModalDrawerTitle>
            </ModalDrawerHeader>
            <ModalDrawerBody>
              <Hidden name="id" />
              ...
              <VStack spacing={4}>
                <Input name="name" label={t("sales:noQuoteReason")} />
