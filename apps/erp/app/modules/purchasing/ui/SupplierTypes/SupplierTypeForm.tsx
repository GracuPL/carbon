...
const SupplierTypeForm = ({
  ...
  useEffect(() => {
    ...
    if (fetcher.state === "loading" && fetcher.data?.data) {
      onClose?.();
      toast.success(t("createdSupplierType"));
    } else if (fetcher.state === "idle" && fetcher.data?.error) {
      toast.error(
        t("failedToCreateSupplierType", { message: fetcher.data.error.message })
      );
    }
      ...
      >
        <ModalDrawerContent>
          ...
          >
            <ModalDrawerHeader>
              <ModalDrawerTitle>
                {isEditing ? t("purchasing:editSupplierType") : t("purchasing:newSupplierType")}
              </ModalDrawerTitle>
            </ModalDrawerHeader>
            <ModalDrawerBody>
              <Hidden name="id" />
              ...
              <VStack spacing={4}>
                <Input name="name" label={t("purchasing:supplierType")} />
                <CustomFormFields table="supplierType" />
              </VStack>
            </ModalDrawerBody>
            <ModalDrawerFooter>
              <HStack>
